# Debugging Guide for STM32F469 Discovery

This guide covers JTAG/SWD debugging for MicroPython firmware on the STM32F469 Discovery board.
...
The STM32F469 Discovery board has an integrated **ST-Link/V2-1 debugger** for JTAG/SWD debugging. This provides:
...
The `disco` tool provides a convenient CLI for common board operations. **For most debugging tasks, you only need the disco tool** - the manual OpenOCD/GDB sections below are reference material for advanced debugging.
...
```nix
{
  description = "STM32F469 development environment";
...
### QSPI Shadow Directories Bricking Device

**Symptoms:**
- Black screen, no USB CDC (REPL unavailable)
- Only ST-LINK VCP visible
- Even old known-working firmware fails to boot
- Flash verify passes but device crashes

**Root Cause:**
QSPI flash contains shadow directories (`/qspi/hosts`, `/qspi/wallets`, etc.) that shadow frozen modules. After MicroPython upgrade, the import system tries to use these empty directories as packages, causing crash before USB init.

**Solution:**
1. Mass erase internal flash: `echo "yes" | scripts/disco flash erase`
   - **WARNING**: This erases bootloader too!
2. Flash FULL firmware from 0x08000000:
   ```bash
   scripts/disco flash program full-image.bin --addr 0x08000000
   ```
3. QSPI will be reformatted on first boot

**Key Lesson:** Flash verify passes but device crashes → issue is in QSPI, not internal flash.

---

### SPI Flash Hang During Settings Load

**Symptoms:**
- Board hangs at "Loading settings..." during boot
- REPL unresponsive
- PC stuck at `0x080dce1e` (spiflash_readblocks_raw)

**Root Cause:**
QSPI read loop waiting for FIFO Threshold Flag (FTF) that never gets set:
```c
while (!(QUADSPI->SR & QUADSPI_SR_FTF)) {  // STUCK HERE
    if (QUADSPI->SR & QUADSPI_SR_TEF) { return -MP_EIO; }
}
```
...
**Debug Steps:**
1. Check QUADSPI->SR register value via GDB
2. Verify flash chip JEDEC ID readable
3. Check if flash was left in deep sleep mode

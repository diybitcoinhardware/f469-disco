...
/**
  ...
  */
static USBH_StatusTypeDef GetLineCoding(USBH_HandleTypeDef *phost, CCID_LineCodingTypeDef *linecoding)
{
 
  phost->Control.setup.b.bmRequestType = USB_D2H | USB_REQ_TYPE_CLASS | \
                              USB_REQ_RECIPIENT_INTERFACE;
  
  phost->Control.setup.b.bRequest = CCID_GET_LINE_CODING;
...
/**
  ...
  * @retval USBH_StatusTypeDef : USB ctl xfer status
  */
static USBH_StatusTypeDef SetLineCoding(USBH_HandleTypeDef *phost, CCID_LineCodingTypeDef *linecodin)
{
  phost->Control.setup.b.bmRequestType = USB_H2D | USB_REQ_TYPE_CLASS | \
                              USB_REQ_RECIPIENT_INTERFACE;
  
  phost->Control.setup.b.bRequest = CCID_SET_LINE_CODING;
  phost->Control.setup.b.wValue.w = 0;
...
* @retval None
*/
USBH_StatusTypeDef  USBH_CCID_GetLineCoding(USBH_HandleTypeDef *phost, CCID_LineCodingTypeDef *linecodin)
{
  CCID_HandleTypeDef *CCID_Handle =  phost->pActiveClass->pData; 
  
  if((phost->gState == HOST_CLASS) ||(phost->gState == HOST_CLASS_REQUEST))
  {
    *linecodin = CCID_Handle->LineCoding;
    return USBH_OK;
  }
  else
  {

...
STATIC mp_obj_t usecp256k1_xonly_pubkey_parse(const mp_obj_t arg){
    maybe_init_ctx();
    mp_buffer_info_t secbuf;
    mp_get_buffer_raise(arg, &secbuf, MP_BUFFER_READ);
    if(secbuf.len != 32){
        mp_raise_ValueError("xonly-pubkey should be 32 bytes long");
        return mp_const_none;
    }
    byte * buf = (byte*)secbuf.buf;
    secp256k1_xonly_pubkey pubkey;
    int res = secp256k1_xonly_pubkey_parse(ctx, &pubkey, secbuf.buf);
    if(!res){
        mp_raise_ValueError("Failed to parse xonly-pubkey");
        return mp_const_none;
    }
    vstr_t vstr;
    vstr_init_len(&vstr, 64);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(usecp256k1_xonly_pubkey_parse_obj, usecp256k1_xonly_pubkey_parse);

// serialize x-only public key
STATIC mp_obj_t usecp256k1_xonly_pubkey_serialize(const mp_obj_t arg){
    maybe_init_ctx();
    mp_buffer_info_t pubbuf;
    mp_get_buffer_raise(arg, &pubbuf, MP_BUFFER_READ);
    if(pubbuf.len != 64){
        mp_raise_ValueError("Pubkey should be 64 bytes long");
        return mp_const_none;
    }
    ...
    memcpy(pubkey.data, pubbuf.buf, 64);
    byte out[32];
    int res = secp256k1_xonly_pubkey_serialize(ctx, out, &pubkey);
    vstr_t vstr;
    vstr_init_len(&vstr, 32);
    memcpy((byte*)vstr.buf, out, 32);
    return mp_obj_new_str_from_vstr(&mp_type_bytes, &vstr);
}

STATIC MP_DEFINE_CONST_FUN_OBJ_1(usecp256k1_xonly_pubkey_serialize_obj, usecp256k1_xonly_pubkey_serialize);

// create public key from private key
STATIC mp_obj_t usecp256k1_xonly_pubkey_create(const mp_obj_t arg){
    maybe_init_ctx();
    mp_buffer_info_t secretbuf;
    mp_get_buffer_raise(arg, &secretbuf, MP_BUFFER_READ);
    if(secretbuf.len != 32){
    ...
    secp256k1_xonly_pubkey pubkey;
    int res = secp256k1_xonly_pubkey_create(ctx, &pubkey, secretbuf.buf);
    if(!res){
        mp_raise_ValueError("Invalid private key");
        return mp_const_none;
    }
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(usecp256k1_xonly_pubkey_create_obj, usecp256k1_xonly_pubkey_create);

STATIC mp_obj_t usecp256k1_xonly_pubkey_from_pubkey(const mp_obj_t arg){
    maybe_init_ctx();
    mp_buffer_info_t pubbuf;
    mp_get_buffer_raise(arg, &pubbuf, MP_BUFFER_READ);
    if(pubbuf.len != 64){
    ...
    secp256k1_xonly_pubkey pubkey;
    int is_negated = 0;
    int res = secp256k1_xonly_pubkey_from_pubkey(ctx, &pubkey, &is_negated, pubbuf.buf);
    if(!res){
...
// tweak private key in place
STATIC mp_obj_t usecp256k1_xonly_seckey_tweak_add(mp_obj_t privarg, const mp_obj_t tweakarg){
    maybe_init_ctx();
    mp_buffer_info_t privbuf;
    mp_get_buffer_raise(privarg, &privbuf, MP_BUFFER_READ);
    if(privbuf.len != 32){
    ...
    int res = secp256k1_xonly_seckey_tweak_add(ctx, privbuf.buf, tweakbuf.buf);
    if(!res){ // never happens according to the API
        mp_raise_ValueError("Failed to tweak the private key");
        return mp_const_none;
    }
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(usecp256k1_xonly_seckey_tweak_add_obj, usecp256k1_xonly_seckey_tweak_add);

// tweak public key in place (add tweak * Generator)
STATIC mp_obj_t usecp256k1_xonly_pubkey_tweak_add(mp_obj_t pubarg, const mp_obj_t tweakarg){
    maybe_init_ctx();
    mp_buffer_info_t pubbuf;
    mp_get_buffer_raise(pubarg, &pubbuf, MP_BUFFER_READ);
    if(pubbuf.len != 64){
    ...
    int is_negated = 0;
    int res = secp256k1_xonly_pubkey_tweak_add(ctx, &pub, &is_negated, tweakbuf.buf);
    if(!res){ // never happens according to the API
        mp_raise_ValueError("Failed to tweak the public key");
        return mp_const_none;
    }
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(usecp256k1_xonly_pubkey_tweak_add_obj, usecp256k1_xonly_pubkey_tweak_add);

// serialize schnorr signature
STATIC mp_obj_t usecp256k1_schnorrsig_serialize(const mp_obj_t arg){
    maybe_init_ctx();
    mp_buffer_info_t buf;
    mp_get_buffer_raise(arg, &buf, MP_BUFFER_READ);
    if(buf.len != 64){
    ...
    vstr_t vstr;
    vstr_init_len(&vstr, 64);
    secp256k1_schnorrsig_serialize(ctx, (byte*)vstr.buf, &sig);
    return mp_obj_new_str_from_vstr(&mp_type_bytes, &vstr);
}

STATIC MP_DEFINE_CONST_FUN_OBJ_1(usecp256k1_schnorrsig_serialize_obj, usecp256k1_schnorrsig_serialize);

// parse schnorr signature
STATIC mp_obj_t usecp256k1_schnorrsig_parse(const mp_obj_t arg){
    maybe_init_ctx();
    mp_buffer_info_t buf;
    mp_get_buffer_raise(arg, &buf, MP_BUFFER_READ);
    if(buf.len != 64){
    ...
    secp256k1_schnorrsig sig;
    int res = secp256k1_schnorrsig_parse(ctx, &sig, (byte*)buf.buf);
    if(!res){
        mp_raise_ValueError("Failed parsing schnorr signature");
        return mp_const_none;
    }
    vstr_t vstr;
    vstr_init_len(&vstr, 64);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(usecp256k1_schnorrsig_parse_obj, usecp256k1_schnorrsig_parse);
...
// sign
STATIC mp_obj_t usecp256k1_schnorrsig_sign(const mp_obj_t msgarg, const mp_obj_t secarg){
    maybe_init_ctx();
    mp_buffer_info_t msgbuf;
    mp_get_buffer_raise(msgarg, &msgbuf, MP_BUFFER_READ);
    if(msgbuf.len != 32){
    ...
    secp256k1_schnorrsig sig;
    int res = secp256k1_schnorrsig_sign(ctx, &sig, msgbuf.buf, secbuf.buf, NULL, NULL);
    if(!res){
        mp_raise_ValueError("Failed to sign");
        return mp_const_none;
    }
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(usecp256k1_schnorrsig_sign_obj, usecp256k1_schnorrsig_sign);
...
STATIC const mp_rom_map_elem_t secp256k1_module_globals_table[] = {
    ...
    { MP_ROM_QSTR(MP_QSTR_xonly_pubkey_parse), MP_ROM_PTR(&usecp256k1_xonly_pubkey_parse_obj) },
    { MP_ROM_QSTR(MP_QSTR_xonly_pubkey_serialize), MP_ROM_PTR(&usecp256k1_xonly_pubkey_serialize_obj) },
    { MP_ROM_QSTR(MP_QSTR_xonly_pubkey_create), MP_ROM_PTR(&usecp256k1_xonly_pubkey_create_obj) },
    { MP_ROM_QSTR(MP_QSTR_xonly_pubkey_from_pubkey), MP_ROM_PTR(&usecp256k1_xonly_pubkey_from_pubkey_obj) },
    { MP_ROM_QSTR(MP_QSTR_xonly_seckey_tweak_add), MP_ROM_PTR(&usecp256k1_xonly_seckey_tweak_add_obj) },
    { MP_ROM_QSTR(MP_QSTR_xonly_pubkey_tweak_add), MP_ROM_PTR(&usecp256k1_xonly_pubkey_tweak_add_obj) },

    { MP_ROM_QSTR(MP_QSTR_schnorrsig_serialize), MP_ROM_PTR(&usecp256k1_schnorrsig_serialize_obj) },
    { MP_ROM_QSTR(MP_QSTR_schnorrsig_parse), MP_ROM_PTR(&usecp256k1_schnorrsig_parse_obj) },
    ...
    // TODO:
    // secp256k1_xonly_pubkey_tweak_test(ctx, pubout, isneg, pubin, tweak)
